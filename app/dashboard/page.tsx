...
// --- MOCK DATA ---
const kpiData = [
  { title: "Active Employees", value: "1,284", trend: "+4.2%", isPositive: true, icon: Users, color: "text-blue-400" },
  { title: "Online Devices", value: "1,150", trend: "-1.1%", isPositive: false, icon: MonitorSmartphone, color: "text-indigo-400" },
  { title: "Avg Productivity", value: "87.4%", trend: "+2.4%", isPositive: true, icon: Activity, color: "text-emerald-400" },
  { title: "Idle Rate", value: "12.6%", trend: "-0.8%", isPositive: true, icon: Clock, color: "text-amber-400" },
  { title: "Risk Score", value: "14/100", trend: "-5.0%", isPositive: true, icon: ShieldAlert, color: "text-rose-400" },
  { title: "Total Work Hours", value: "8,420h", trend: "+12.5%", isPositive: true, icon: Zap, color: "text-blue-500" },
];

const insights = [
  { id: 1, text: "Productivity drops by 18% consistently after 2:00 PM across Engineering.", severity: "warning" },
  { id: 2, text: "High idle rate (22%) detected in the remote Sales department today.", severity: "alert" },
  { id: 3, text: "Overall system risk score decreased following the new software policy.", severity: "success" }
];

const activities = [
  { id: 1, user: "Ahmed M.", action: "Started secure session", time: "Just now", status: "success" },
  { id: 2, user: "Device #8842", action: "Went offline unexpectedly", time: "2 min ago", status: "error" },
  { id: 3, user: "Sarah J.", action: "Triggered prolonged idle alert", time: "15 min ago", status: "warning" },
  { id: 4, user: "System", action: "Automated daily report generated", time: "1 hour ago", status: "info" },
  { id: 5, user: "David K.", action: "Logged in from new IP", time: "2 hours ago", status: "warning" },
];

// --- COMPONENTS ---

const SidebarItem = ({ icon: Icon, label, active = false }: { icon: React.ComponentType<{ size?: number; className?: string }>; label: string; active?: boolean }) => (
  <button 
  ...
  >
    ...
    {active && <div className="ml-auto w-1 h-5 bg-blue-500 rounded-full shadow-[0_0_8px_rgba(0,102,255,0.8)]" />}
  </button>
);
...
// --- MOCK CHARTS ---
// These simulate high-end chart libraries using pure SVG and Tailwind
const AreaChart = () => (
...
export default function App() {
  return (
    <div className="h-screen w-full bg-[#050811] text-white flex overflow-hidden font-sans selection:bg-blue-500/30">
      {/* Background Grid Overlay (Matches WPF) */}
      <div className="absolute inset-0 pointer-events-none opacity-[0.02]" 
           style={{ backgroundImage: 'linear-gradient(white 1px, transparent 1px), linear-gradient(90deg, white 1px, transparent 1px)', backgroundSize: '40px 40px' }} />

      {/* --- SIDEBAR --- */}
      <aside className="w-72 border-r border-white/5 bg-[#080B14]/80 backdrop-blur-xl flex flex-col z-20">
        {/* Logo Area */}
        <div className="h-20 flex items-center px-8 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-lg tracking-widest text-white leading-tight">ORION<span className="text-blue-500">GUARD</span></h1>
              <p className="text-[9px] text-slate-500 font-medium tracking-widest uppercase">Intelligence System</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 no-scrollbar">
          <div className="text-xs font-semibold text-slate-600 mb-4 px-4 tracking-wider uppercase">Main Dashboard</div>
          <SidebarItem icon={LayoutDashboard} label="Overview" active={true} />
          <SidebarItem icon={Users} label="Employees" />
          <SidebarItem icon={MonitorSmartphone} label="Devices" />
          <SidebarItem icon={ActivitySquare} label="Analytics" />
          
          <div className="text-xs font-semibold text-slate-600 mb-4 mt-8 px-4 tracking-wider uppercase">Monitoring</div>
          <SidebarItem icon={ImageIcon} label="Screenshots" />
          <SidebarItem icon={Bell} label="Alerts" />
          
          <div className="text-xs font-semibold text-slate-600 mb-4 mt-8 px-4 tracking-wider uppercase">System</div>
        ...
        <div className="p-4 border-t border-white/5 bg-white/2">
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/5 border border-white/10 cursor-pointer hover:bg-white/10 transition">
            ...
            <div className="flex-1 overflow-hidden">
              <div className="text-sm font-medium truncate">Admin Portal</div>
              <div className="text-xs text-slate-400 truncate">HQ Server Node</div>
            </div>
      ...
      <main className="flex-1 flex flex-col h-full relative z-10">
        ...
        <header className="h-20 border-b border-white/5 bg-[#080B14]/50 backdrop-blur-md flex items-center justify-between px-8 z-20">
          <div className="flex items-center gap-6">
            <h2 className="text-xl font-semibold text-white tracking-wide">Enterprise Overview</h2>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative hidden md:block group">
              ...
              <input 
                ...
                className="bg-[#0B101E] border border-white/10 rounded-lg pl-10 pr-4 py-2 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/50 focus:bg-white/5 transition w-64"
              />
            </div>
            ...
            <button className="relative p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-lg transition">
              <Bell size={20} />
              <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-rose-500 rounded-full border-2 border-[#050811]"></span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 rounded-2xl bg-[#0B101E] border border-white/5 p-6 relative overflow-hidden flex flex-col">
                ...
                <div className="flex justify-between items-center mb-6 z-10">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">7-day performance curve across all nodes</p>
                  </div>
              ...
              <div className="rounded-2xl bg-linear-to-b from-[#0B101E] to-[#0A0D18] border border-white/5 p-6 flex flex-col relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 mb-6 relative z-10">
                  ...
                  <h3 className="text-lg font-semibold text-white">Orion AI Insights</h3>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-8">
              ...
              <div className="rounded-2xl bg-[#0B101E] border border-white/5 p-6 flex flex-col">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-lg font-semibold text-white">Department Activity</h3>
                ...
                <div className="flex justify-center gap-6 mt-4 pt-4 border-t border-white/5 text-xs text-slate-400">
                  ...
                  <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-slate-700"></div> Idle</div>
                </div>
              </div>

              {/* Application Distribution (Simulated Donut via SVG) */}
              <div className="rounded-2xl bg-[#0B101E] border border-white/5 p-6 flex flex-col">
                <h3 className="text-lg font-semibold text-white mb-6">App Category Distribution</h3>
                <div className="flex-1 flex items-center justify-center relative">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    ...
                    <span className="text-xs text-slate-500">Apps Logged</span>
                  </div>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                  <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#0066FF]"></div> <span className="text-slate-300">Dev Tools (60%)</span></div>
                  <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#6366F1]"></div> <span className="text-slate-300">Comm. (25%)</span></div>
                  <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#14B8A6]"></div> <span className="text-slate-300">Design (10%)</span></div>
                  <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-[#1E293B]"></div> <span className="text-slate-300">Other (5%)</span></div>
                </div>
              </div>
              ...
              <div className="rounded-2xl bg-[#0B101E] border border-white/5 p-6 flex flex-col">
                ...
                <div className="flex-1 flex flex-col gap-0 relative">
                  ...
                  <div className="absolute left-2.5 top-2 bottom-2 w-px bg-white/10 z-0"></div>
